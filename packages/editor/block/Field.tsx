...
export interface Stage {
  ...
  startDragBlock(e: DragState, item: BlockItem): [BlockItem, number, number, number][];
    ...
    return (
      ...
      >
        ...
        <BlockDropdown
          ...
          canApply={false}
          funcLib={blockItem.stage.getFuncLib()}
        >
          {nameNode}
        </BlockDropdown>
      </DragDropDiv>
...
export abstract class BaseBlockItem extends DataRendererItem<XYWRenderer> {
  ...
  watchDesc() {
    ...
    this.descScope = this.stage.getFuncLib();
    if (typeof this.isValue === 'string') {
      this.conn.watchDesc(this.isValue, getDescLib(this.isValue, this.descScope), this.descListener);
    }
  }

  onDescScopeChanged() {
    if (this.isValue != null && this.descScope !== this.stage.getFuncLib()) {
      this.watchDesc();
    }
  }
...
export class BlockItem extends BaseBlockItem {
  ...
  destroy() {
    if (this._syncChild) {
      this._syncChild.syncParent = null;
      this._syncChild = null;
    }
    if (this._syncParent) {
      this._syncParent.syncChild = null;
