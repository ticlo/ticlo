...
import {coreFunctions, DescListener} from '../block/FunctionGroup.js';
...
class ServerDescWatcher extends ServerRequest implements DescListener {
  ...
  constructor(conn: ServerConnection, id: string) {
    ...
    this.id = id;
    this.connection = conn;
    this.pendingIds = new Set(coreFunctions.getAllFunctionIds());
    coreFunctions.listenDesc(this);
    this.connection.addSend(this);
  }
  ...
  getSendingData(): {data: DataMap; size: number} {
    ...
    for (const id of this.pendingIds) {
      const [desc, size] = coreFunctions.getDescToSend(id);
      if (desc) {
        changes.push(desc);
        totalSize += size;
      } else {
  ...
  close() {
    coreFunctions.unlistenDesc(this);
  }
}
  ...
  addBlock({path, data, findName}: {path: string; data?: DataMap; findName?: boolean}): string | DataMap {
    ...
    if (property) {
      ...
      if (typeof funcId === 'string' && data) {
        ...
        const desc = coreFunctions.getDescToSend(funcId)[0];
