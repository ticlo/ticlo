...
export class FastifyServerFunction extends BaseFunction<Block> {
  ...
  requestHandler = async (basePath: string, req: FastifyRequest, res: FastifyReply) => {
    ...
    const emitTask = () => {
      let request = new FastifyHttpRequest(modifiedReq as any, res, basePath);
  ...
  service: RouteService = escapedObject(`fastify-server-${serviceId.next(10)}`, {
