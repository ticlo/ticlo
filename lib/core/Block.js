...
Block.prototype.load = function (map) {
    if (_blockGeneration > 0) {
        throw 'concurrent block loading';
    }
    _blockGeneration = 1 - _blockGeneration;
    if (_blockGeneration >= Number.MAX_SAFE_INTEGER) {
        // TODO in this case some block won't be triggered, fix it
        _blockGeneration = 1;
    }
    this._load(map);
    _blockGeneration = -_blockGeneration;
};
...
Block.prototype.trigger = function () {
    if (!this._loopRef) {
        if (this._gen !== _blockGeneration || this._logic.initRun) {
            Loop.addBlock(this);
        }
    }
};
